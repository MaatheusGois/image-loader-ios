...
@implementation LoadImageFramePool
...
- (instancetype)init {
    ...
    if (self) {
        ...
        _fetchQueue.name = @"com.hackemist.LoadImageFramePool.fetchQueue";
#if SD_UIKIT
...
+ (instancetype)registerProvider:(id<SDAnimatedImageProvider>)provider {
    ...
    LoadImageFramePool *framePool = [self.providerFramePoolMap objectForKey:provider];
    if (!framePool) {
        framePool = [[LoadImageFramePool alloc] init];
