 ...
 */
@interface LoadImageCacheConfig : NSObject <NSCopying>

/**
 Gets the default cache config used for shared instance or initialization when it does not provide any cache config. Such as `LoadImageCache.sharedImageCache`.
 ...
 */
@property (nonatomic, class, readonly, nonnull) LoadImageCacheConfig *defaultCacheConfig;
...
/*
 * The option to control weak memory cache for images. When enable, `LoadImageCache`'s memory cache will use a weak maptable to store the image at the same time when it stored to memory, and get removed at the same time.
 ...
 * When enabling this option, we will sync back the image from weak maptable to strong cache during next time top level `btg_setImage` function call.
 ...
 */
@property (assign, nonatomic) LoadImageCacheConfigExpireType diskCacheExpireType;
...
/**
 * The dispatch queue attr for ioQueue. You can config the QoS and concurrent/serial to internal IO queue. The ioQueue is used by LoadImageCache to access read/write for disk data.
