...
- (void)storeImage:(nullable UIImage *)image
    ...
    if (!data && image) {
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
            ...
            NSData *data = [[SDImageCodersManager sharedManager] encodedDataWithImage:image format:format options:context[ImageLoaderContextImageEncodeOptions]];
            dispatch_async(self.ioQueue, ^{
...
- (nullable UIImage *)imageFromDiskCacheForKey:(nullable NSString *)key options:(SDImageCacheOptions)options context:(nullable ImageLoaderContext *)context {
    if (!key) {
        return nil;
    }
    ...
    if (context[ImageLoaderContextStoreCacheType]) {
        SDImageCacheType cacheType = [context[ImageLoaderContextStoreCacheType] integerValue];
...
- (nullable UIImage *)imageFromCacheForKey:(nullable NSString *)key options:(SDImageCacheOptions)options context:(nullable ImageLoaderContext *)context {
...
- (nullable UIImage *)diskImageForKey:(nullable NSString *)key data:(nullable NSData *)data options:(SDImageCacheOptions)options context:(ImageLoaderContext *)context {
    if (!data) {
        return nil;
    }
...
- (nullable SDImageCacheToken *)queryCacheOperationForKey:(nullable NSString *)key options:(SDImageCacheOptions)options context:(nullable ImageLoaderContext *)context done:(nullable SDImageCacheQueryCompletionBlock)doneBlock {
    return [self queryCacheOperationForKey:key options:options context:context cacheType:SDImageCacheTypeAll done:doneBlock];
}

- (nullable SDImageCacheToken *)queryCacheOperationForKey:(nullable NSString *)key options:(SDImageCacheOptions)options context:(nullable ImageLoaderContext *)context cacheType:(SDImageCacheType)queryCacheType done:(nullable SDImageCacheQueryCompletionBlock)doneBlock {
    if (!key) {
        if (doneBlock) {
            doneBlock(nil, nil, SDImageCacheTypeNone);
        }
...
#pragma mark - Helper
+ (ImageLoaderOptions)imageOptionsFromCacheOptions:(SDImageCacheOptions)cacheOptions {
    ImageLoaderOptions options = 0;
    if (cacheOptions & SDImageCacheScaleDownLargeImages) options |= ImageLoaderScaleDownLargeImages;
    if (cacheOptions & SDImageCacheDecodeFirstFrameOnly) options |= ImageLoaderDecodeFirstFrameOnly;
    if (cacheOptions & SDImageCachePreloadAllFrames) options |= ImageLoaderPreloadAllFrames;
    if (cacheOptions & SDImageCacheAvoidDecodeImage) options |= ImageLoaderAvoidDecodeImage;
    if (cacheOptions & SDImageCacheMatchAnimatedImageClass) options |= ImageLoaderMatchAnimatedImageClass;
...
- (id<ImageLoaderOperation>)queryImageForKey:(NSString *)key options:(ImageLoaderOptions)options context:(nullable ImageLoaderContext *)context completion:(nullable SDImageCacheQueryCompletionBlock)completionBlock {
...
- (id<ImageLoaderOperation>)queryImageForKey:(NSString *)key options:(ImageLoaderOptions)options context:(nullable ImageLoaderContext *)context cacheType:(SDImageCacheType)cacheType completion:(nullable SDImageCacheQueryCompletionBlock)completionBlock {
    SDImageCacheOptions cacheOptions = 0;
    if (options & ImageLoaderQueryMemoryData) cacheOptions |= SDImageCacheQueryMemoryData;
    if (options & ImageLoaderQueryMemoryDataSync) cacheOptions |= SDImageCacheQueryMemoryDataSync;
    if (options & ImageLoaderQueryDiskDataSync) cacheOptions |= SDImageCacheQueryDiskDataSync;
    if (options & ImageLoaderScaleDownLargeImages) cacheOptions |= SDImageCacheScaleDownLargeImages;
    if (options & ImageLoaderAvoidDecodeImage) cacheOptions |= SDImageCacheAvoidDecodeImage;
    if (options & ImageLoaderDecodeFirstFrameOnly) cacheOptions |= SDImageCacheDecodeFirstFrameOnly;
    if (options & ImageLoaderPreloadAllFrames) cacheOptions |= SDImageCachePreloadAllFrames;
    if (options & ImageLoaderMatchAnimatedImageClass) cacheOptions |= SDImageCacheMatchAnimatedImageClass;
    
    return [self queryCacheOperationForKey:key options:cacheOptions context:context cacheType:cacheType done:completionBlock];
}

- (void)storeImage:(UIImage *)image imageData:(NSData *)imageData forKey:(nullable NSString *)key cacheType:(SDImageCacheType)cacheType completion:(nullable ImageLoaderNoParamsBlock)completionBlock {
...
- (void)removeImageForKey:(NSString *)key cacheType:(SDImageCacheType)cacheType completion:(nullable ImageLoaderNoParamsBlock)completionBlock {
    switch (cacheType) {
...
- (void)clearWithCacheType:(SDImageCacheType)cacheType completion:(ImageLoaderNoParamsBlock)completionBlock {
    switch (cacheType) {
        case SDImageCacheTypeNone: {
            if (completionBlock) {
                completionBlock();
