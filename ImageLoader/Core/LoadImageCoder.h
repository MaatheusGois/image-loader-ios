...
/**
 ...
 @note Which means, this just forward to `encodedDataWithFrames([LoadImageFrame(image: image, duration: 0], image._imageLoopCount))`
...
/**
 ...
 @param loopCount The final animated image loop count. 0 means infinity loop. This config ignore each frame's `_imageLoopCount`
 @param format The image format to encode, you should note `LoadImageFormatUndefined` format is also  possible
 @param options A dictionary containing any encoding options. Pass @{LoadImageCoderEncodeCompressionQuality: @(1)} to specify compression quality.
 @return The encoded image data
 */
