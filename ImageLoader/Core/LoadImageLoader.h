...
typedef void(^LoadImageLoaderProgressBlock)(NSInteger receivedSize, NSInteger expectedSize, NSURL * _Nullable targetURL);
typedef void(^LoadImageLoaderCompletedBlock)(UIImage * _Nullable image, NSData * _Nullable data, NSError * _Nullable error, BOOL finished);

#pragma mark - Context Options

/**
 A `UIImage` instance from `ImageLoaderManager` when you specify `ImageLoaderRefreshCached` and image cache hit.
 This can be a hint for image loader to load the image from network and refresh the image from remote location if needed. If the image from remote location does not change, you should call the completion with `ImageLoaderErrorCacheNotModified` error. (UIImage)
 @note If you don't implement `ImageLoaderRefreshCached` support, you do not need to care about this context option.
 */
FOUNDATION_EXPORT ImageLoaderContextOption _Nonnull const ImageLoaderContextLoaderCachedImage;

#pragma mark - Helper method

/**
 This is the built-in decoding process for image download from network or local file.
 @note If you want to implement your custom loader with `requestImageWithURL:options:context:progress:completed:` API, but also want to keep compatible with ImageLoader's behavior, you'd better use this to produce image.
 ...
 */
FOUNDATION_EXPORT UIImage * _Nullable LoadImageLoaderDecodeImageData(NSData * _Nonnull imageData, NSURL * _Nonnull imageURL, ImageLoaderOptions options, ImageLoaderContext * _Nullable context);

/**
 This is the built-in decoding process for image progressive download from network. It's used when `ImageLoaderProgressiveLoad` option is set. (It's not required when your loader does not support progressive image loading)
 @note If you want to implement your custom loader with `requestImageWithURL:options:context:progress:completed:` API, but also want to keep compatible with ImageLoader's behavior, you'd better use this to produce image.
 ...
 */
FOUNDATION_EXPORT UIImage * _Nullable LoadImageLoaderDecodeProgressiveImageData(NSData * _Nonnull imageData, NSURL * _Nonnull imageURL, BOOL finished,  id<ImageLoaderOperation> _Nonnull operation, ImageLoaderOptions options, ImageLoaderContext * _Nullable context);
 ...
 */
FOUNDATION_EXPORT id<SDProgressiveImageCoder> _Nullable LoadImageLoaderGetProgressiveCoder(id<ImageLoaderOperation> _Nonnull operation);
 ...
 */
FOUNDATION_EXPORT void LoadImageLoaderSetProgressiveCoder(id<ImageLoaderOperation> _Nonnull operation, id<SDProgressiveImageCoder> _Nullable progressiveCoder);
...
/**
 ...
 If you want to implement custom loader for image download from network or local file, you just need to concentrate on image data download only. After the download finish, call `LoadImageLoaderDecodeImageData` or `LoadImageLoaderDecodeProgressiveImageData` to use the built-in decoding process and produce image (Remember to call in the global queue). And finally callback the completion block.
 ...
 @note It's your responsibility to load the image in the desired global queue(to avoid block main queue). We do not dispatch these method call in a global queue but just from the call queue (For `ImageLoaderManager`, it typically call from the main queue).
*/
@protocol LoadImageLoader <NSObject>
 ...
 */
- (nullable id<ImageLoaderOperation>)requestImageWithURL:(nullable NSURL *)url
                                                options:(ImageLoaderOptions)options
                                                context:(nullable ImageLoaderContext *)context
                                               progress:(nullable LoadImageLoaderProgressBlock)progressBlock
                                              completed:(nullable LoadImageLoaderCompletedBlock)completedBlock;


/**
 Whether the error from image loader should be marked indeed un-recoverable or not.
 If this return YES, failed URL which does not using `ImageLoaderRetryFailed` will be blocked into black list. Else not.
