...
#if SD_UIKIT
static inline UIImage *LoadImageDecodeUIKit(UIImage *image) {
...
static inline UIImage *LoadImageDecodeAndScaleDownUIKit(UIImage *image, CGSize destResolution) {
...
static inline BOOL LoadImageSupportsHardwareHEVCDecoder(void) {
...
static LoadImageCoderDecodeSolution kDefaultDecodeSolution = LoadImageCoderDecodeSolutionAutomatic;
...
/*
 * Defines the maximum size in MB of the decoded image when the flag `ImageLoaderScaleDownLargeImages` is set
...
@implementation LoadImageCoderHelper

+ (UIImage *)animatedImageWithFrames:(NSArray<LoadImageFrame *> *)frames {
    ...
    [frames enumerateObjectsUsingBlock:^(LoadImageFrame * _Nonnull frame, NSUInteger idx, BOOL * _Nonnull stop) {
    ...
    CFStringRef imageUTType = [NSData _UTTypeFromImageFormat:LoadImageFormatGIF];
...
+ (NSArray<LoadImageFrame *> *)framesFromAnimatedImage:(UIImage *)animatedImage {
    if (!animatedImage) {
        return nil;
    }
    
    NSMutableArray<LoadImageFrame *> *frames;
    ...
    [animatedImages enumerateObjectsUsingBlock:^(UIImage * _Nonnull image, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if ([image isEqual:previousImage]) {
            ...
            LoadImageFrame *frame = [LoadImageFrame frameWithImage:previousImage duration:avgDuration * repeatCount];
    ...
    for (size_t i = 0; i < frameCount; i++) {
        ...
        LoadImageFrame *frame = [LoadImageFrame frameWithImage:frameImage duration:frameDuration];
        [frames addObject:frame];
    }
#endif
...
+ (UIImage *)decodedImageWithImage:(UIImage *)image {
    ...
    } else if (decodeSolution == LoadImageCoderDecodeSolutionUIKit) {
    ...
    if (decodeSolution == LoadImageCoderDecodeSolutionCoreGraphics) {
...
+ (UIImage *)decodedAndScaledDownImageWithImage:(UIImage *)image limitBytes:(NSUInteger)bytes {
    ...
    // on iOS7, do not forget to call [[LoadImageCache sharedImageCache] clearMemory];
    @autoreleasepool {
        // device color space
...
+ (LoadImageCoderDecodeSolution)defaultDecodeSolution {
    return kDefaultDecodeSolution;
}

+ (void)setDefaultDecodeSolution:(LoadImageCoderDecodeSolution)defaultDecodeSolution {
