...
@interface LoadImageIOCoderFrame : NSObject
...
@implementation LoadImageIOCoderFrame
@end

@implementation LoadImageIOAnimatedCoder {
    ...
    NSArray<LoadImageIOCoderFrame *> *_frames;
    BOOL _finished;
...
+ (LoadImageFormat)imageFormat {
    @throw [NSException exceptionWithName:NSInternalInconsistencyException
                                   reason:[NSString stringWithFormat:@"For `LoadImageIOAnimatedCoder` subclass, you must override %@ method", NSStringFromSelector(_cmd)]
                                 userInfo:nil];
}
...
+ (BOOL)canDecodeFromFormat:(LoadImageFormat)format {
...
+ (UIImage *)createFrameAtIndex:(NSUInteger)index source:(CGImageSourceRef)source scale:(CGFloat)scale preserveAspectRatio:(BOOL)preserveAspectRatio thumbnailSize:(CGSize)thumbnailSize lazyDecode:(BOOL)lazyDecode animatedImage:(BOOL)animatedImage {
    ...
    if (!createFullImage) {
        if (preserveAspectRatio) {
            ...
            CGImageRef scaledImageRef = [LoadImageCoderHelper CGImageCreateScaled:imageRef size:thumbnailSize];
            if (scaledImageRef) {
    ...
    if (!lazyDecode) {
        if (!isDecoded) {
            ...
            CGImageRef decodedImageRef = [LoadImageCoderHelper CGImageCreateDecoded:imageRef];
            if (decodedImageRef) {
                CGImageRelease(imageRef);
    ...
    UIImageOrientation imageOrientation = [LoadImageCoderHelper imageOrientationFromEXIFOrientation:exifOrientation];
    UIImage *image = [[UIImage alloc] initWithCGImage:imageRef scale:scale orientation:imageOrientation];
#else
    UIImage *image = [[UIImage alloc] initWithCGImage:imageRef scale:scale orientation:exifOrientation];
#endif
    CGImageRelease(imageRef);
    image._isDecoded = isDecoded;
...
- (BOOL)canDecodeFromData:(nullable NSData *)data {
    return ([NSData _imageFormatForImageData:data] == self.class.imageFormat);
}
...
- (UIImage *)decodedImageWithData:(NSData *)data options:(nullable LoadImageCoderOptions *)options {
    ...
    CGFloat scale = 1;
    NSNumber *scaleFactor = options[LoadImageCoderDecodeScaleFactor];
    if (scaleFactor != nil) {
        scale = MAX([scaleFactor doubleValue], 1);
    }
    
    CGSize thumbnailSize = CGSizeZero;
    NSValue *thumbnailSizeValue = options[LoadImageCoderDecodeThumbnailPixelSize];
    ...
    NSNumber *preserveAspectRatioValue = options[LoadImageCoderDecodePreserveAspectRatio];
    if (preserveAspectRatioValue != nil) {
        preserveAspectRatio = preserveAspectRatioValue.boolValue;
    }
    
    BOOL lazyDecode = YES; // Defaults YES for static image coder
    NSNumber *lazyDecodeValue = options[LoadImageCoderDecodeUseLazyDecoding];
    ...
    if (thumbnailSize.width == 0 || thumbnailSize.height == 0) {
        ...
        if (imageRep) {
            ...
            animatedImage._imageFormat = self.class.imageFormat;
            return animatedImage;
        }
    }
#endif
    
    NSString *typeIdentifierHint = options[LoadImageCoderDecodeTypeIdentifierHint];
    if (!typeIdentifierHint) {
        ...
        NSString *fileExtensionHint = options[LoadImageCoderDecodeFileExtensionHint];
    ...
    NSNumber *limitBytesValue = options[LoadImageCoderDecodeScaleDownLimitBytes];
    ...
    BOOL decodeFirstFrame = [options[LoadImageCoderDecodeFirstFrameOnly] boolValue];
    if (decodeFirstFrame || frameCount <= 1) {
        ...
        animatedImage._imageLoopCount = loopCount;
    }
    animatedImage._imageFormat = self.class.imageFormat;
    CFRelease(source);
...
- (UIImage *)incrementalDecodedImageWithOptions:(LoadImageCoderOptions *)options {
    NSCParameterAssert(_incremental);
    UIImage *image;
    ...
    if (_width + _height > 0) {
        ...
        if (image) {
            image._imageFormat = self.class.imageFormat;
        }
    }
...
- (NSData *)encodedDataWithImage:(UIImage *)image format:(LoadImageFormat)format options:(nullable LoadImageCoderOptions *)options {
    ...
    NSArray<LoadImageFrame *> *frames = [LoadImageCoderHelper framesFromAnimatedImage:image];
    if (!frames || frames.count == 0) {
        LoadImageFrame *frame = [LoadImageFrame frameWithImage:image duration:0];
        frames = @[frame];
    }
    return [self encodedDataWithFrames:frames loopCount:image._imageLoopCount format:format options:options];
}
...
- (NSData *)encodedDataWithFrames:(NSArray<LoadImageFrame *> *)frames loopCount:(NSUInteger)loopCount format:(LoadImageFormat)format options:(LoadImageCoderOptions *)options {
...
#if SD_UIKIT || SD_WATCH
    CGImagePropertyOrientation exifOrientation = [LoadImageCoderHelper exifOrientationFromImageOrientation:image.imageOrientation];
#else
    ...
    if (options[LoadImageCoderEncodeCompressionQuality]) {
        compressionQuality = [options[LoadImageCoderEncodeCompressionQuality] doubleValue];
    }
    properties[(__bridge NSString *)kCGImageDestinationLossyCompressionQuality] = @(compressionQuality);
    CGColorRef backgroundColor = [options[LoadImageCoderEncodeBackgroundColor] CGColor];
    if (backgroundColor) {
        properties[(__bridge NSString *)kCGImageDestinationBackgroundColor] = (__bridge id)(backgroundColor);
    }
    CGSize maxPixelSize = CGSizeZero;
    NSValue *maxPixelSizeValue = options[LoadImageCoderEncodeMaxPixelSize];
    ...
    NSUInteger maxFileSize = [options[LoadImageCoderEncodeMaxFileSize] unsignedIntegerValue];
    if (maxFileSize > 0) {
    ...
    if (options[LoadImageCoderEncodeEmbedThumbnail]) {
        embedThumbnail = [options[LoadImageCoderEncodeEmbedThumbnail] boolValue];
    }
    properties[(__bridge NSString *)kCGImageDestinationEmbedThumbnail] = @(embedThumbnail);
    
    BOOL encodeFirstFrame = [options[LoadImageCoderEncodeFirstFrameOnly] boolValue];
...
- (BOOL)scanAndCheckFramesValidWithImageSource:(CGImageSourceRef)imageSource {
    ...
    NSMutableArray<LoadImageIOCoderFrame *> *frames = [NSMutableArray arrayWithCapacity:frameCount];
    for (size_t i = 0; i < frameCount; i++) {
        LoadImageIOCoderFrame *frame = [[LoadImageIOCoderFrame alloc] init];
        frame.index = i;
