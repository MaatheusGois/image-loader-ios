...
- (void)_drawInRect:(CGRect)rect context:(CGContextRef)context scaleMode:(LoadImageScaleMode)scaleMode clipsToBounds:(BOOL)clips {
...
- (nullable UIImage *)_resizedImageWithSize:(CGSize)size scaleMode:(LoadImageScaleMode)scaleMode {
    if (size.width <= 0 || size.height <= 0) return nil;
    SDGraphicsImageRendererFormat *format = [[SDGraphicsImageRendererFormat alloc] init];
    format.scale = self.scale;
    SDGraphicsImageRenderer *renderer = [[SDGraphicsImageRenderer alloc] initWithSize:size format:format];
    UIImage *image = [renderer imageWithActions:^(CGContextRef  _Nonnull context) {
        [self _drawInRect:CGRectMake(0, 0, size.width, size.height) context:context scaleMode:scaleMode clipsToBounds:NO];
    }];
    return image;
}

- (nullable UIImage *)_croppedImageWithRect:(CGRect)rect {
...
- (nullable UIImage *)_roundedCornerImageWithRadius:(CGFloat)cornerRadius corners:(SDRectCorner)corners borderWidth:(CGFloat)borderWidth borderColor:(nullable UIColor *)borderColor {
    SDGraphicsImageRendererFormat *format = [[SDGraphicsImageRendererFormat alloc] init];
    format.scale = self.scale;
...
#else
            NSBezierPath *path = [NSBezierPath _bezierPathWithRoundedRect:CGRectInset(rect, borderWidth, borderWidth) byRoundingCorners:corners cornerRadius:cornerRadius];
#endif
            [path closePath];
...
#else
            NSBezierPath *path = [NSBezierPath _bezierPathWithRoundedRect:strokeRect byRoundingCorners:corners cornerRadius:strokeRadius];
#endif
            [path closePath];
...
- (nullable UIImage *)_rotatedImageWithAngle:(CGFloat)angle fitSize:(BOOL)fitSize {
...
- (nullable UIImage *)_flippedImageWithHorizontal:(BOOL)horizontal vertical:(BOOL)vertical {
    size_t width = self.size.width;
    size_t height = self.size.height;
...
- (nullable UIImage *)_tintedImageWithColor:(nonnull UIColor *)tintColor {
...
- (nullable UIColor *)_colorAtPoint:(CGPoint)point {
    CGImageRef imageRef = NULL;
    // CIImage compatible
#if SD_UIKIT || SD_MAC
    if (self.CIImage) {
...
- (nullable NSArray<UIColor *> *)_colorsWithRect:(CGRect)rect {
    CGImageRef imageRef = NULL;
    // CIImage compatible
#if SD_UIKIT || SD_MAC
    if (self.CIImage) {
...
- (nullable UIImage *)_blurredImageWithRadius:(CGFloat)blurRadius {
...
#if SD_UIKIT || SD_MAC
- (nullable UIImage *)_filteredImageWithFilter:(nonnull CIFilter *)filter {
    CIImage *inputImage;
    if (self.CIImage) {
