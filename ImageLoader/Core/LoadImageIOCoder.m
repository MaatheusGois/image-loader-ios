...
@implementation LoadImageIOCoder {
...
+ (instancetype)sharedCoder {
    static LoadImageIOCoder *coder;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        coder = [[LoadImageIOCoder alloc] init];
    });
    return coder;
}
...
- (UIImage *)decodedImageWithData:(NSData *)data options:(nullable LoadImageCoderOptions *)options {
    ...
    // Check vector format
    if ([NSData btg_imageFormatForImageData:data] == LoadImageFormatPDF) {
        ...
        if (thumbnailSize.width == 0 || thumbnailSize.height == 0) {
            ...
            if (imageRep) {
                ...
                image.btg_imageFormat = LoadImageFormatPDF;
                return image;
            }
        }
#endif
    ...
    LoadImageFormat imageFormat = [NSData btg_imageFormatFromUTType:uttype];
    ...
    image.btg_imageFormat = imageFormat;
    return image;
}
...
- (UIImage *)incrementalDecodedImageWithOptions:(LoadImageCoderOptions *)options {
    ...
    if (_width + _height > 0) {
        ...
        if (image) {
            ...
            image.btg_imageFormat = [NSData btg_imageFormatFromUTType:uttype];
        }
    }
...
- (NSData *)encodedDataWithImage:(UIImage *)image format:(LoadImageFormat)format options:(nullable LoadImageCoderOptions *)options {
    ...
    if (format == LoadImageFormatUndefined) {
        BOOL hasAlpha = [LoadImageCoderHelper CGImageContainsAlpha:imageRef];
        if (hasAlpha) {
            format = LoadImageFormatPNG;
        } else {
            format = LoadImageFormatJPEG;
        }
    }
