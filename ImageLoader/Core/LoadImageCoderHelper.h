...
typedef NS_ENUM(NSUInteger, LoadImageCoderDecodeSolution) {
    ...
    LoadImageCoderDecodeSolutionAutomatic,
    ...
    LoadImageCoderDecodeSolutionCoreGraphics,
 ...
 */
+ (UIImage * _Nullable)animatedImageWithFrames:(NSArray<LoadImageFrame *> * _Nullable)frames;
 ...
 */
+ (NSArray<LoadImageFrame *> * _Nullable)framesFromAnimatedImage:(UIImage * _Nullable)animatedImage NS_SWIFT_NAME(frames(from:));
...
/**
 Control the default force decode solution. Available solutions  in `LoadImageCoderDecodeSolution`.
 @note Defaults to `LoadImageCoderDecodeSolutionAutomatic`, which prefers to use UIKit for JPEG/HEIF, and fallback on CoreGraphics. If you want control on your hand, set the other solution.
 */
@property (class, readwrite) LoadImageCoderDecodeSolution defaultDecodeSolution;
